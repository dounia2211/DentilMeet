...
class PatientModel {
    ...
    public function create($full_name, $email, $passwordHash, $phone, $birth_date, $gender) {
        $stmt = $this->pdo->prepare("
            INSERT INTO patient (full_name, email, password, phone, birth_date, gender, created_at)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->execute([$full_name, $email, $passwordHash, $birthdate, $age, $gender]);
